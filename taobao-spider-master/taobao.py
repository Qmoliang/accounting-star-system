...
def Usage():
    print ('数据库大作业  taobao-spider 使用指南:')
    print ('    -h    --help:    显示帮助信息.')
    print ('    -v    --ver:    显示脚本版本')
...
def spider(num,keyWord,fileName):
    ...
    file.write('num,商品ID,商品名称,价格,地址,销量,好评,中评,差评\n')
    for k in range(0,limit):        
        ...
        for i in range(0,x) :    #把列表的数据保存到文件中
            ...
            try:
                file.write(str(k*44+i+1)+','+pid[i]+','+title[i]+','+price[i]+','+loc[i]+','+sales[i]+','+positive+','+middle+','+negative+'\n')
            except: 
                file.write(str(k*44+i+1)+','+pid[i]+','+title[i]+','+price[i]+','+loc[i]+','+"0人付款"+','+positive+','+middle+','+negative+'\n')
    print (pb(1,1))
    file.close()
def main(argv):
    try:
